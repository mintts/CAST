...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("CAST")
    updateDisplayUnit();
}
...
{
    QString ret = QString("CAST:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non CAST unit input in lnReqAmount, we generate the URI with BKC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::CAST, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
